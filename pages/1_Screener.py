...
_SCHEDULE_FILE = os.path.join(_PROJECT_ROOT, "cache", "daily_schedule.json")
_SCAN_LOG_FILE = os.path.join(_PROJECT_ROOT, "cache", "daily_scan_log.txt")
...
def _update_github_cron(time_str_et: str):
    ...
    try:
        ...
        workflow_path = os.path.join(_PROJECT_ROOT,
...
def _run_daily_scan_background(scan_size: int, phone: str = ""):
    ...
    try:
        ...
        # 1. Fetch universe
        _bg_config = _load_schedule()
        _bg_universe = _bg_config.get("universe", "All US Large-Cap ($1B+)")
        symbols = fetch_universe(_bg_universe, scan_size)
            ...
            try:
                ...
                if config.get("enabled"):
                    ...
                    if current_time == target_time and current_date != last_triggered_date:
                        ...
                        script = os.path.join(_PROJECT_ROOT, "daily_scan.py")
...
with st.sidebar:
    ...
    st.caption("Use the sidebar navigation to go back to Home")
    ...
    _tg_creds_file = os.path.join(_PROJECT_ROOT, "cache", "tg_creds.json")
    _saved_tg = {}
    try:
    ...
    daily_universe_name = "All US Large-Cap ($1B+)"
    daily_tg = False
    if daily_enabled:
        daily_universe_name = st.selectbox(
            "Daily Universe",
            list(UNIVERSE_OPTIONS.keys())[:-1],  # Exclude "Custom"
            index=list(UNIVERSE_OPTIONS.keys()).index(saved_config.get("universe", "All US Large-Cap ($1B+)")),
            help="Which stock universe to scan daily",
        )
...
if run_daily_now:
    ...
    daily_uni_name = config.get("universe", "All US Large-Cap ($1B+)")
    st.info(f"Running daily scan — **{daily_scan_size}** stocks from **{daily_uni_name}**...")
    progress_bar = st.progress(0, text="Fetching stock universe...")

    daily_tickers = fetch_universe(daily_uni_name, daily_scan_size)
